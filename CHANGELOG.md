...
- Ability to output qr code images in `print`, `ca`, and `sign` modes for `nebula-cert` (#297)
...
- ICMP ping requests can be responded to when the `tun.disabled` is `true` (#342)
...
- End to end testing (#425, #427, #428)
...
- Better ordered startup to enable `sshd`, `stats`, and `dns` subsystems to listen on nebula (#375)

### Fixed

- No longer report handshake packets under `lost` in stats (#331)
