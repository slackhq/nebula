...
func signCert(args []string, out io.Writer, errOut io.Writer, pr PasswordReader) error {
	...
	if *sf.networks != "" {
		for _, rs := range strings.Split(*sf.networks, ",") {
			//TODO: CERT-V2 error on duplicates? Mainly only addr matters, having two of the same addr in the same or different prefix space is strange
			rs := strings.Trim(rs, " ")
			if rs != "" {
	...
	if *sf.unsafeNetworks != "" {
		//TODO: CERT-V2 error on duplicates?
