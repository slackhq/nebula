...
func BenchmarkHotPath(b *testing.B) {
	ca, _, caKey, _ := cert.NewTestCaCert(cert.Version1, cert.Curve_CURVE25519, time.Now(), time.Now().Add(10*time.Minute), nil, nil, []string{})
...
func TestRehandshakingRelays(t *testing.T) {
	...
	_, _, myNextPrivKey, myNextPEM := cert.NewTestCert(cert.Version1, cert.Curve_CURVE25519, ca, caKey, "relay", time.Now(), time.Now().Add(5*time.Minute), relayVpnIpNet, nil, []string{"new group"})
...
func TestRehandshaking(t *testing.T) {
	...
	_, _, myNextPrivKey, myNextPEM := cert.NewTestCert(cert.Version1, cert.Curve_CURVE25519, ca, caKey, "me", time.Now(), time.Now().Add(5*time.Minute), myVpnIpNet, nil, []string{"new group"})
...
func TestRehandshakingLoser(t *testing.T) {
	...
	_, _, theirNextPrivKey, theirNextPEM := cert.NewTestCert(cert.Version1, cert.Curve_CURVE25519, ca, caKey, "them", time.Now(), time.Now().Add(5*time.Minute), theirVpnIpNet, nil, []string{"their new group"})
