...
func (f *Interface) getOrHandshake(vpnIp uint32) *HostInfo {
	...
	if ci == nil {
		...
		if err != nil {
			...
			f.l.WithError(err).WithField("vpnIp", IntIp(vpnIp)).Error("Failed to get a PSK KDF")
			return hostinfo
		}

		ci, err = f.newConnectionState(f.l, true, p)
		if err != nil {
			f.l.WithError(err).WithField("vpnIp", IntIp(vpnIp)).Error("Failed to get a connection state")
			return hostinfo
		}
