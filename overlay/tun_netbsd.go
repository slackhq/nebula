//go:build netbsd && !e2e_testing
// +build netbsd,!e2e_testing
...
func newTunFromFd(_ *config.C, _ *logrus.Logger, _ int, _ []netip.Prefix) (*wgTun, error) {
	return nil, fmt.Errorf("newTunFromFd not supported on NetBSD")
}
...
func newTun(c *config.C, l *logrus.Logger, vpnNetworks []netip.Prefix, _ bool) (*wgTun, error) {
	...
	// Create NetBSD-specific route manager
	t.routeManager = &tun{}
...
func (rm *tun) SetDefaultRoute(t *wgTun, cidr netip.Prefix) error {
	// On NetBSD, routes are set via ifconfig and route commands
	return nil
}
...
func (rm *tun) AddRoutes(t *wgTun, logErrors bool) error {
	...
	for _, r := range routes {
		...
		if r.Metric > 0 {
			// NetBSD doesn't support route metrics directly like Linux
			t.l.WithField("route", r).Warn("Route metrics are not fully supported on NetBSD")
		}
...
func (rm *tun) NewMultiQueueReader(t *wgTun) (io.ReadWriteCloser, error) {
	// NetBSD doesn't support multi-queue TUN devices in the same way as Linux
