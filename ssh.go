...
func sshListHostMap(hostMap *HostMap, a interface{}, w sshd.StringWriter) error {
	...
	sort.Slice(hm, func(i, j int) bool {
		...
		binary.BigEndian.PutUint32(vpnI, netaddrTouint32(hm[i].VpnIP))
		binary.BigEndian.PutUint32(vpnJ, netaddrTouint32(hm[j].VpnIP))

		return bytes.Compare(vpnI, vpnJ) < 0
	})
...
func sshListLighthouseMap(lightHouse *LightHouse, a interface{}, w sshd.StringWriter) error {
	...
	sort.Slice(addrMap, func(i, j int) bool {
		...
		binary.BigEndian.PutUint32(vpnI, netaddrTouint32(addrMap[i].VpnIP))
		binary.BigEndian.PutUint32(vpnJ, netaddrTouint32(addrMap[j].VpnIP))
...
func sshQueryLighthouse(ifce *Interface, fs interface{}, a []string, w sshd.StringWriter) error {
	...
	parsedIp, err := netaddr.ParseIP(a[0])
	if err != nil {
		return w.WriteLine(fmt.Sprintf("The provided vpn ip could not be parsed: %s", a[0]))
	}

	var cm *CacheMap
	rl := ifce.lightHouse.Query(parsedIp, ifce)
	if rl != nil {
		cm = rl.CopyCache()
	}
...
func sshCloseTunnel(ifce *Interface, fs interface{}, a []string, w sshd.StringWriter) error {
	...
	hostInfo, err := ifce.hostMap.QueryVpnIP(parsedIp)
	if err != nil {
...
func sshCreateTunnel(ifce *Interface, fs interface{}, a []string, w sshd.StringWriter) error {
	...
	hostInfo, _ := ifce.hostMap.QueryVpnIP(parsedIp)
	if hostInfo != nil {
		return w.WriteLine(fmt.Sprintf("Tunnel already exists"))
	}

	hostInfo, _ = ifce.handshakeManager.pendingHostMap.QueryVpnIP(parsedIp)
	if hostInfo != nil {
	...
	hostInfo = ifce.handshakeManager.AddVpnIP(parsedIp)
	if addr != nil {
		hostInfo.SetRemote(addr)
	}
	ifce.getOrHandshake(parsedIp)
