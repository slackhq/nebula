...
const genericTimerCacheMax = 50000

type GenericTimerWheel struct {
	// Current tick
	current int
...
type GenericTimeoutList struct {
	Head *GenericTimeoutItem
	Tail *GenericTimeoutItem
}

// Represents an item within a tick
type GenericTimeoutItem struct {
	Item interface{}
	Next *GenericTimeoutItem
}
...
func NewGenericTimerWheel(min, max time.Duration) *GenericTimerWheel {
	...
	tw := GenericTimerWheel{
		wheelLen:      wLen,
		wheel:         make([]*GenericTimeoutList, wLen),
		tickDuration:  min,
		wheelDuration: max,
		expired:       &GenericTimeoutList{},
	}
...
func (tw *GenericTimerWheel) Add(v interface{}, timeout time.Duration) *GenericTimeoutItem {
	tw.lock.Lock()
...
func (tw *GenericTimerWheel) Purge() interface{} {
	tw.lock.Lock()
	defer tw.lock.Unlock()
...
func (tw *GenericTimerWheel) findWheel(timeout time.Duration) (i int) {
...
func (tw *GenericTimerWheel) advance(now time.Time) {
	tw.lock.Lock()
	defer tw.lock.Unlock()
